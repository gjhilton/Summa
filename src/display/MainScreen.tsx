...
const SwipeContext = React.createContext<SwipeContextValue>({
	openId: null,
	setOpenId: () => {},
});
function useSwipeContext() {
	return React.useContext(SwipeContext);
}
...
function ActionStrip({
	...
	return (
		<ActionStripWrapper
			...
			hasRevealButton={canHover || undefined}
			data-no-print
		>
			<ActionButton
				tabIndex={-1}
				...
				onClick={() => {
					if (canDelete && window.confirm('Delete this row?')) {
						onRemove?.();
						onClose();
					}
				}}
			...
			<ActionButton
				tabIndex={-1}
				onClick={() => {
					if (window.confirm('Clear this item?')) {
						onClearItem?.();
						onClose();
					}
				}}
...
/** Creates a gesture tracker object that does NOT use React hooks. */
function makeSwipeGesture(): SwipeHandlers {
	...
	function onTouchMove(e: React.TouchEvent) {
		...
		if (Math.abs(dy) > Math.abs(dx)) isVertical = true;
	}
	function onTouchEnd(
		e: React.TouchEvent,
		onOpen: () => void,
		...
		if (dx < -40) onOpen();
		else if (dx > 20) onClose();
	}
	return { onTouchStart, onTouchMove, onTouchEnd };
}

// One gesture tracker per item, created once (stable across renders via module-level map keyed by id).
// This is safe because gesture state is ephemeral (resets each touch sequence) and non-reactive.
const gestureMap = new WeakMap<object, SwipeHandlers>();
function getSwipeGesture(key: object): SwipeHandlers {
	let g = gestureMap.get(key);
	if (!g) {
		g = makeSwipeGesture();
		gestureMap.set(key, g);
	}
	return g;
}

const swipeItemKeys = new Map<string, object>();
function getSwipeKey(id: string): object {
	let k = swipeItemKeys.get(id);
	if (!k) {
		k = {};
		swipeItemKeys.set(id, k);
	}
	return k;
}
...
export function SwipeableItem({
	...
	const swipe = getSwipeGesture(getSwipeKey(id));
	...
	return (
		<Item
			...
			onDuplicate={onDuplicate}
			onClearItem={onClearItem}
			onTouchStart={!canHover ? swipe.onTouchStart : undefined}
			onTouchMove={!canHover ? swipe.onTouchMove : undefined}
			onTouchEnd={
				!canHover
					? e => swipe.onTouchEnd(e, onReveal, onClose)
					: undefined
			}
		>
			{children}
...
function renderExplanation(line: AnyLineState): React.ReactNode {
	...
	switch (result.type) {
		case 'lsd':
			return (
				<>
					{renderExplanationTerms(result.terms)} = {result.totalPence}
					<SupD />
				</>
			);
		case 'extended':
			return (
				<>
					{renderExplanationTerms(result.unitCostTerms)} ={' '}
					{result.basePence}
					<SupD /> unit cost × {result.quantity} = {result.totalPence}
					<SupD />
				</>
			);
		case 'total':
...
function renderErrorExplanation(line: AnyLineState): React.ReactNode | null {
	switch (line.itemType) {
		case ItemType.LINE_ITEM: {
			const labels = fieldErrorLabels(line.fieldErrors);
			if (labels.length === 0) return null;
			return (
				<>
					this item has an error: only Roman numerals allowed in{' '}
					{formatFieldList(labels)}
				</>
			);
		}
		case ItemType.EXTENDED_ITEM: {
			const lsdLabels = fieldErrorLabels(line.fieldErrors);
			const quantityMissing = line.quantityError && !line.quantity.trim();
			const invalidFields = [
				...lsdLabels,
				...(line.quantityError && line.quantity.trim()
					? ['quantity']
					: []),
			];
			...
			if (invalidFields.length > 0)
				parts.push(
					`only Roman numerals allowed in ${formatFieldList(invalidFields)}`
				);
		...
		case ItemType.SUBTOTAL_ITEM:
			...
			return (
				<>this item has an error: a sub-item contains invalid input</>
			);
	}
}

function renderTotalExplanation(
	...
	return (
		<>
			{result.totalPence}
			<SupD /> = {renderExplanationTerms(result.terms)}
		</>
	);
}
...
export const CurrencyField = ({
	...
	<Label data-currency-field>
		<TextInput
			...
			error={error || undefined}
		/>
		<FieldAnnotation sup dim={value === '0' || undefined}>
			{label}
		</FieldAnnotation>
	</Label>
);
...
export const Currency = ({
	...
	<Equally data-currency displayOnly={!editable || undefined}>
		<CurrencyField
			...
			editable={editable}
			value={values.l}
			onChange={v => onFieldChange?.('l', v)}
			error={fieldErrors?.l || undefined}
		/>
		<CurrencyField
			label="s"
			editable={editable}
			value={values.s}
			onChange={v => onFieldChange?.('s', v)}
			error={fieldErrors?.s || undefined}
		/>
		<CurrencyField
			label="d"
			editable={editable}
...
const EditLink = ({ onClick }: { onClick?: () => void }) => (
	<EditLinkButton type="button" onClick={onClick}>
		edit
	</EditLinkButton>
);
...
export const ItemUnit = ({
	...
	>
		...
		{explanation && (
			<ExplanationRow
				data-no-print
				isError={explanationIsError || undefined}
			>
				{explanation}
			</ExplanationRow>
		)}
...
export const AddItemBar = ({
	...
	<AddBar data-no-print>
		{advanced ? (
			<>
				<Button icon={PlusIcon} onClick={onAddUnit}>
					item
				</Button>
				<Button icon={PlusIcon} onClick={onAddExtended}>
					extended
				</Button>
				<Button icon={PlusIcon} onClick={onAddSubtotal}>
					subtotal
				</Button>
			</>
		) : (
...
export const FooterEdit = ({
	...
	<ScreenFooter
		...
		controls={
			<>
				<Toggle
					id="explain-calculations"
					label="explain calculations"
			...
			<PageWidth>
				<SubHeaderTopRow>
					...
					<Button
						variant="danger"
						onClick={() => {
							if (window.confirm('Clear all items?')) onClear?.();
						}}
					>
						clear
					</Button>
...
export const HeaderEdit = ({
	...
	<ScreenHeader>
		<Button variant="primary" onClick={onSaveClick} disabled={hasError}>
			export
		</Button>
...
function RenderLine({
	...
	const shared = {
		...
		canDelete,
		explanation,
		explanationIsError: explanationIsError || undefined,
		error: line.error || undefined,
		onRemove: () => handlers.onRemoveLine(line.id),
		onDuplicate: () => handlers.onDuplicateLine(line.id),
		onClearItem: () => handlers.onClearItem(line.id),
		onTitleChange: (v: string) => handlers.onTitleChange(line.id, v),
	};
	switch (line.itemType) {
		case ItemType.LINE_ITEM:
			return (
				<ItemUnit
					...
					onFieldChange={(f, v) =>
						handlers.onFieldChange(line.id, f, v)
					}
				/>
			);
		case ItemType.EXTENDED_ITEM:
			return (
				<ItemExtended
					...
					onQuantityChange={v =>
						handlers.onQuantityChange(line.id, v)
					}
				/>
			);
		case ItemType.SUBTOTAL_ITEM:
			return (
				<ItemSubTotal
					...
					onEdit={() => handlers.onEditSubtotal(line.id)}
				/>
			);
	}
}
...
export function ListOfItems({
	...
	return (
		<ItemListSection>
			{heading ? (
				<ItemListHeading>{heading}</ItemListHeading>
			) : (
				<ItemListTopBorder />
			)}
			<SortableContext
			...
			>
				{lines.map(line => {
					const errorExplanation = showExplanation
						? renderErrorExplanation(line)
						: null;
					const explanation =
						errorExplanation ??
						(showExplanation ? renderExplanation(line) : null);
					return (
						<SortableLine key={line.id} id={line.id}>
							<RenderLine
								...
								explanationIsError={!!errorExplanation}
								handlers={handlers}
							/>
						</SortableLine>
					);
			...
			<ItemTotal
				...
				explanation={
					showExplanation
						? renderTotalExplanation(totalDisplay, totalPence)
						: null
				}
				itemCount={showExplanation ? lines.length : undefined}
...
export type DialogRef = React.RefObject<HTMLDialogElement | null>;

export function SummaDialog({
	dialogRef,
	onClose,
	title,
	children,
	...
	function handleBackdropClick(e: React.MouseEvent<HTMLDialogElement>) {
		...
		if (rect && isClickOutsideRect(rect, e.clientX, e.clientY)) onClose();
	}
...
export interface SaveModalUIProps {
	dialogRef: DialogRef;
	filename: string;
...
export interface LoadModalUIProps {
	dialogRef: DialogRef;
...
interface RootHeaderProps {
	isSubLevel: boolean;
	breadcrumbs: MainScreenProps['breadcrumbs'];
	onNavigate: MainScreenProps['onNavigate'];
	onDone?: MainScreenProps['onDone'];
	onClear: MainScreenProps['onClear'];
	onUndo: MainScreenProps['onUndo'];
	canUndo: MainScreenProps['canUndo'];
	hasError?: MainScreenProps['hasError'];
	onSaveClick: () => void;
	onLoadClick: () => void;
}
	...
	return (
		<ScreenContainer>
			<MainScreenHeader
				...
				canUndo={canUndo}
				hasError={hasError}
				onSaveClick={onSaveOpen}
				onLoadClick={onLoadOpen}
			/>
			<SwipeProvider openId={swipeOpenId} setOpenId={onSwipeOpenIdChange}>
				<ListOfItems
					lines={lines}
					totalDisplay={totalDisplay}
			...
			{!isSubLevel && (
				<>
					<SaveModalUI
						...
						onFilenameChange={onSaveFilenameChange}
						onClose={onSaveClose}
						onSave={onSave}
					/>
					<LoadModalUI
						dialogRef={loadDialogRef}
