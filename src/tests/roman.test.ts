...
describe('romanToInteger', () => {
	...
	it('converts subtractive iv to 4', () =>
		expect(romanToInteger('iv')).toBe(4));
	it('converts subtractive ix to 9', () =>
		expect(romanToInteger('ix')).toBe(9));
	it('converts subtractive xl to 40', () =>
		expect(romanToInteger('xl')).toBe(40));
	it('converts subtractive xc to 90', () =>
		expect(romanToInteger('xc')).toBe(90));
	it('converts subtractive cd to 400', () =>
		expect(romanToInteger('cd')).toBe(400));
	it('converts subtractive cm to 900', () =>
		expect(romanToInteger('cm')).toBe(900));

	it('converts additive iiii to 4', () =>
		expect(romanToInteger('iiii')).toBe(4));
	it('converts additive viii to 8', () =>
		expect(romanToInteger('viii')).toBe(8));

	it('converts mcmxcix to 1999', () =>
		expect(romanToInteger('mcmxcix')).toBe(1999));
	it('converts mmxxvi to 2026', () =>
		expect(romanToInteger('mmxxvi')).toBe(2026));

	it('is case-insensitive (XIV)', () =>
		expect(romanToInteger('XIV')).toBe(14));
	it('is case-insensitive (MCMXCIX)', () =>
		expect(romanToInteger('MCMXCIX')).toBe(1999));
});
...
describe('isValidRoman', () => {
	...
	it('rejects il (invalid subtractive)', () =>
	...
	it('rejects non-roman chars', () =>
		expect(isValidRoman('abc')).toBe(false));
	it('rejects spaces', () => expect(isValidRoman('i i')).toBe(false));
	it('rejects j (unnormalized early modern)', () =>
		expect(isValidRoman('iiij')).toBe(false));
